...
The simplest version of the bear case boils down to this:

> _"every company that doesn't touch physical reality is just expensive middleware between a problem and a solution"._

AI offers pure digital output at near-zero marginal cost, around the clock. So if your product is organised thinking, and thinking can now be replicated at near-zero cost, then the moats that software companies built over the past two decades, e.g. near-zero marginal cost, strong pricing power, minimal capital requirements, are straight-up eroding. Under this framework, ServiceNow is a workflow tool that will be disintermediated by AI agents that can orchestrate tasks directly.

The trigger was concrete. For instance, when Anthropic launched Claude Cowork, $285 billion vanished from SaaS valuations in roughly 48 hours. The market concluded that AI agents could replace entire categories of per-seat knowledge work software. And ServiceNow, which sells workflow automation on a subscription basis, was squarely in the crosshairs. Its trailing P/E compressed from ~155x to ~63x in under a year. It wasn't alone either, its peers Salesforce, Workday, and the broader BVP Emerging Cloud Index all sold off in sympathy, but few such companies of this scale and quality have been marked down this hard on narrative alone.

There's a contradiction worth noting here too. As Bank of America pointed out earlier in February, the sell-off is paradoxically pricing in two scenarios that cannot simultaneously be true: AI capex deteriorating because the returns aren't there, and AI being so pervasive it renders established software obsolete. If AI is powerful enough to disrupt ServiceNow, the infrastructure spending behind it can't simultaneously be collapsing. And the same strategists warning that software is being commoditised are, in the same breath, identifying the categories that I believe will actually go on to thrive.
...
I remember reading Clive Thompson's book, _'Smarter Than You Think'_ years ago. But the first chapter about 'centaur chess' has stayed with me in particular.
...
Thompson's framework surfaces a useful filter: does this software gain leverage from AI augmentation, or does AI simply execute the task the software was wrapping? For narrow, well-defined tasks like document review or report generation, the answer is substitution; the AI executes the whole task and the human is out of the loop.

AI agents are brilliant at processing information, but they're only as useful as the data they can access and work with, they still need a filing cabinet. And it's the companies that own those filing cabinets, with decades of proprietary data, identity governance, audit trails, cross-system permissions and workflow context, that hold the real value. This is 'data gravity' in practice. It's far more practical to bring the AI to where the data already lives than to migrate it somewhere new.

So for platforms that provide that infrastructure - deep data integrations, compliance requirements and cross-system dependencies for example, the answer is augmentation. AI operates _through_ them, not in place of them.

I'd place ServiceNow firmly in the second category. You can swap models underneath, and you can change cloud providers. But migrating your entire agent orchestration, identity governance, and asset visibility infrastructure? That's ripping out the foundations. ServiceNow isn't competing with Claude. It's the platform Claude will be licensed into.

That technical lock-in is reinforced by something the sell-off narrative overlooks entirely - the sheer psychological entrenchment of status quo bias. Enterprises that have spent years embedding ServiceNow across HR, IT, security and finance aren't evaluating alternatives from a blank slate. They're anchored to their existing investment. The switching costs are real, but the psychological entrenchment runs deeper than the technical lock-in. It's the same compromise effect I've written about before. Once an organisation has committed to a platform, the cumulative investment generates its own justification. Every new workflow built on ServiceNow makes the next one more likely to be built there too.

Meanwhile, the market is caught in its own feedback loop running in the opposite direction. The classic 'price drives narrative' reflexivity trap. When a stock drops, people reverse-engineer reasons why it should be down, and the "AI kills SaaS" thesis has provided a conveniently tidy explanation. The sell-off validates the thesis, which attracts more sellers, which further validates it, and the consensus hardens not because the analysis has deepened but because the price action keeps confirming the story. It's hard to avoid the conclusion that much of what passes for bear commentary on ServiceNow right now is post-hoc rationalisation of recent price movement.

#### The Moat

So if ServiceNow isn't the kind of software AI replaces, how is it positioning itself for what comes next? The abstract arguments I've laid out - augmentation over substitution, data gravity and centaur roles, only really matter if the company is actually executing on them.

To understand why its AI strategy has credibility, we first have to appreciate the depth of its existing footprint.

ServiceNow functions as the system of record for IT service management, HR workflows, security operations, and customer service across almost 8,400 enterprises, including 85-90% of the Fortune 500. As I've already touched on, its moat is rooted primarily in switching costs, and the 98% renewal rate tells you everything you need to know about how much it costs to leave.
...
It's still early days, and projecting a straight line from here would be naïve. That said, the early financial signals are encouraging. Now Assist has crossed $600M in annual contract value, roughly doubling year-over-year, and is tracking toward $1B by the end of 2026. When analysts describe this traction as "healthy but not enough," they're measuring it against the scale of the existing $13.3B subscription base. Whether that pace holds remains to be seen, but it's a credible start for a product that's only been in the market for eighteen months.
...
There are two ways a stock can be cheap. It can be cheap relative to something, e.g. its own history, its peers, the market. Or it can be cheap in an absolute sense, meaning it returns enough cash to its owners to justify the price on its own terms, without needing the story to get better.

The first kind of cheapness is what most investors are measuring when they look at ServiceNow's 50% drawdown and call it a buying opportunity. The second is what a value framework demands before it will agree. They aren't mutually exclusive. A stock can be relatively cheap and absolutely cheap at the same time, but assuming one implies the other is where valuation mistakes begin.
...
| Component              | Amount    |
|------------------------|-----------|
| Market capitalisation  | $107.8B   |
| Less: cash & securities| ($10.1B)  |
| Plus: debt             | $1.5B     |
...
ServiceNow's income statement tells a version of the same story. In FY2025, GAAP operating income was $1.8 billion, with stock-based compensation totalling $2.0 billion. The company handed its employees _more_ in equity than it earned for its shareholders. To put that another way: ServiceNow's employees were compensated more in stock than the entire business earned from operations. That doesn't mean the business is being deceptive - after all, SBC is how the entire software industry competes for talent, and ServiceNow is no outlier among its peers. But it does mean that the free cash flow figures flatter the underlying economics, and on that point, the parallel with Burry's era feels undeniable.

Where the parallel diverges is growth. Adobe, Cisco, and Intel were indeed profitable companies when Burry was writing, but their growth engines were stalling while dilution continued. The distinction to make is that ServiceNow today is still compounding revenue at 21% with accelerating forward commitments. The SBC is arguably buying something real, which is the talent that is scaling the business. And there are signs of improving discipline too. SBC as a percentage of revenue has compressed from 17.9% in FY2023 to 14.7% in FY2025, suggesting the company is outgrowing its dilution rather than being consumed by it.

But outgrowing dilution is not the same as eliminating it, and the free cash flow still reflects the gap. The $4.6 billion figure is as high as it is precisely because SBC isn't a cash outflow. The cost is being borne by shareholders through dilution, not by the business through the bank account. Adjust for it and the picture shifts materially.

| Metric                      | Reported   | SBC-adjusted |
|-----------------------------|------------|--------------|
| Operating cash flow         | $5.4B      |              |
| Less: capital expenditure   | ($0.9B)    |              |
| **Free cash flow**          | **$4.6B**  |              |
| Less: stock-based comp      |            | ($2.0B)      |
| **Adjusted free cash flow** |            | **$2.6B**    |
| EV / FCF                    | ~21.6x     | ~38x         |
| Implied FCF yield           | ~4.5%      | ~2.6%        |

At 38x adjusted free cash flow, the implied yield drops to 2.6%, which is roughly half the risk-free rate. Burry's threshold of a 10% annual return after dilution required something closer to 10x. By that yardstick, the gap between what ServiceNow appears to cost and what it actually costs is substantial.

#### The Buyback Treadmill

The natural follow-up to the SBC question is whether buybacks are offsetting the dilution. Management certainly frames it that way. On the Q4 FY2025 earnings call, CEO McDermott announced a $5 billion share repurchase authorisation with an immediate $2 billion accelerated buyback, positioning it as a sign of confidence: _"We are one of the few companies totally in control of our own destiny"_.

The 10-K tells a more nuanced story though. In FY2025, ServiceNow issued 11.9 million shares to employees through stock option exercises, RSU vestings, and its employee stock purchase plan. It repurchased 10.3 million shares for $1.8 billion. That's a net increase of 1.6 million shares from compensation alone, before accounting for the 13.2 million shares issued to acquire Moveworks. Total shares outstanding rose from 1,032 million to 1,047 million. The company nearly _tripled_ its buyback spend from the prior year's $696 million and the share count still went up.
...
ServiceNow isn't at that scale of abuse, and it's worth separating the signal from the noise. Excluding the Moveworks acquisition, organic dilution from SBC was just 1.6 million shares, so the buyback program offset 87% of employee issuance. The annual dilution rate on an organic basis is roughly 0.15%, which is modest by software industry standards. The buybacks are doing their job managing dilution, but that is precisely the point; they're just managing it, not returning capital. And as SBC grows, the buyback budget has to grow with it just to keep the share count flat.
...
The distinction wasn't quality. It was _price_. Apple had the margin of safety. It was cash-rich, out of favour, and trading below what the business was demonstrably worth. Amazon might have been the superior business in hindsight to us now, but at the prices on offer at the time, Burry couldn't see a floor beneath him.
...
|------------------------|------------|------------------------------------|
| Current FCF            | $4.6B      | FY2025 actual                      |
| Enterprise value       | $99.2B     | Current market price               |
| Discount rate          | 10%        | Standard equity hurdle rate        |
| Terminal growth        | 3%         | Roughly inflation after year 10    |
| Terminal FCF multiple  | 25x        | Mature high-quality software       |
| **Implied FCF growth** | **~8-9%**  | **What the market is pricing in**  |
...
So who's committing capital? The executives who know this business better than any analyst, framework, or model ever could, and it's obvious they're not hedging.

CEO Bill McDermott has committed approximately $23 million of personal capital to purchasing ServiceNow shares. $20 million was already pledged in late January, alongside a commitment to lead the company through 2030, and a further $3 million purchased on February 27 at prevailing market prices, the earliest date he could buy without triggering short-swing profit rules under Section 16. These are open-market purchases with after-tax personal funds, not stock grants or options exercises. McDermott called it _"a once-in-a-generation moment with ServiceNow"_, the kind of statement that's easy to dismiss from any CEO, less so from one who's already put the money behind it.

But it goes beyond just the CEO. On February 13, five senior executives - the CFO, the Vice Chairman, the Chief People Officer, and the company's Special Counsel among them, all simultaneously terminated their Rule 10b5-1 trading plans, cancelling all future pre-planned stock sales.

For the record, these are pre-scheduled, automated selling programmes that executives set up months in advance to avoid any appearance of insider trading. Cancelling them is not something done lightly; it forfeits future liquidity and invites scrutiny. Cancelling them en masse, during a 50% drawdown, while the CEO is simultaneously buying with personal capital, is as close to a coordinated signal of conviction as securities law permits. Whatever the quantitative framework says about the multiple, the people with the deepest understanding of this business are betting that it's wrong.
...
This analysis started with a qualitative conviction and then tested it against the most unforgiving quantitative framework I could find. Burry's lens is the most honest place to stress-test any investment thesis - strip the story away, ignore the narrative, and ask what the numbers actually say. Applied to ServiceNow, they say the stock isn't cheap; not on an absolute basis, not after adjusting for stock-based compensation, and certainly not by the standards Burry himself would apply.
