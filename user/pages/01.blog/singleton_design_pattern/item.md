...
My first brush with a singleton was in some lacklustre lecture at college, presumably duly-noted a few slides into a class on 'Software Design Patterns'. Docile and unremarkable then, singletons have since returned with a vengeance.
...
So to clarify, I'm not advocating against using singletons, more so against their overuse. It's important to scrutinise their application, since so often their inherent drawbacks outweigh any benefit gained.
...
In the class above, it's impossible to call the constructor because it's private. That's a feature, not a bug, in order to prevent direct calls with the `new` operator.

Instead, the user must use the defined `GetInstance()` method which returns the same single object to the client.

!! The `QueueManager` class has two responsibilities then: ensuring there's only one single instance _and_ managing a global access point to that instance. This is a violation of the [Single Responsibility Principle](https://en.wikipedia.org/wiki/Single-responsibility_principle)
...
Since `QueueManager` is defined as a singleton, the dependencies are inadvertently hidden (i.e. not visible from function/constructor parameters).
...
This coupling limits the modularity of the code, complicating any extension of the interface in the future.

If say, we wanted to overload with a new `double receive_data()` API function, we'd also need to modify the underlying `QueueManager` dependency to enable this change.
...
A good interface should be defined to have loose coupling and be independent of the implementation. This is what enables the underlying interfaced component to change, without causing knock-on effects downstream.
...
Hence my emphasis on scrutinising the choice of a singleton in the first place. Otherwise, you're left inflicting needless pain on yourself for no good reason. 
...
!!! It's worth noting that C++11 guarantees thread-safe initialisation of function-local statics (sometimes called Meyer's Singleton). This addresses the specific initialisation race described above, but does not make the singleton's _usage_ thread-safe; concurrent access to mutable state within the singleton still requires synchronisation.

The broader point remains: singletons introduce threading concerns that wouldn't exist if the object were scoped and injected normally.
...
If object instantiation is required, we could define `QueueManager` as a normal (_service_) class. Then instantiate a single object under the right scope, without requiring a singleton to enforce it, rather than restricting the program to all the constraints of having a singleton in the first place.
...
With this strategy, even if the queue implementation completely changes, the interface is entirely decoupled, meaning any changes to the implementation have no effect on the interface or further downstream.
...
Singletons are not intrinsically bad, but their applicability is far more niche than their prevalence suggests. The pattern conflates two concerns, enforcing a single instance and providing global access, and the consequences of that conflation propagate outward: hidden dependencies, tight coupling, impaired testability, and threading complications that wouldn't arise under a more disciplined design.

Each of the alternatives proposed above addresses these shortcomings with increasing rigour. Static helpers eliminate the class overhead entirely. Dependency injection makes the dependency explicit and mockable. Inversion of control decouples the interface from the implementation altogether. In most cases, the question is not whether these alternatives are better, but why the singleton was chosen in the first place.
